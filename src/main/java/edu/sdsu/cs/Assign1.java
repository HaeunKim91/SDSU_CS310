...
public final class Assign1 {
    ...
    private Assign1() {
        ...
        executor.submit(new Thread(Assign1::writeFingerprint));
            Future<List<String>> results = executor.submit(() -> ListTimer.timeList(new ArrayList<>(), values));

        Future<List<String>> llResults = executor.submit(() -> ListTimer.timeList(new LinkedList<>(), values));
        ...
        try {
            ...
            writeFile(llResults.get(), TIMING_LL_FILE);
